...
// use jni::objects::{JByteArray, JLongArray, JObject, JObjectArray, JClass, JList};

// use jni::sys::{jboolean, jbyteArray, jint, jlong, jobjectArray, jvalue};
...
// #[no_mangle]
// pub unsafe extern "C" fn Java_org_extism_sdk_JniWrapper_extism_1plugin_1new<'local>(
//     env: &mut JNIEnv<'local>,
//     _class: JClass<'local>,
//     ctx: jlong,
//     wasm: JObject<'local>,
//     functions: jobjectArray,
//     nFunctions: jint,
//     withWASI: jboolean
// ) -> jint {

//     let t = env.get_field(wasm, "sources", "l").unwrap().l().unwrap();
//     JList::from_env(env, &t).unwrap();


//     // let data: Vec<u8> = env.convert_byte_array(wasm).unwrap();
//     // let wasm_data: *const u8 = data.as_slice().as_ptr() as *const u8;

//     let mut arr = MaybeUninit::<[ExtismFunction; 0]>::uninit();

//     // (wasmSize as Size) as i32
//     // extism_plugin_new(
//     //     ctx as *mut Context,
//     //     wasm_data,
//     // let a = data.len() as Size;
//     // let b = arr.as_mut_ptr() as *mut *const ExtismFunction;
//     // let c = nFunctions as Size;
//     // let d = withWASI != 0;
//     // )
